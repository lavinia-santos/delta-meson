...
def do_eos_delta():
    ...
    # 
        ...
        result = subprocess.run("./beta-eq-eos21", shell=True)
            #result
...
# 
        result = subprocess.run("./tov22", shell=True)
        result
...
def do_properties_delta():
    ...
    for i in range(0, num_lines):
       with open(temp_input_file, "w") as tempfile:
        ...
        # Execute the external command
        # result = subprocess.run("./properties_eos21", shell=True)
        result = subprocess.run("./esym_eos21", shell=True)
